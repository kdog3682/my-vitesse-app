...
var johnsonEdition = ({ s, offset, resolveTSType }) => {
  ...
  return {
    walkCall(node, parent) {
      ...
      if (parent?.type !== "VariableDeclarator" || parent.id.type !== "Identifier")
        throw new Error(
          `A variable must be used to receive the return value of ${DEFINE_PROP2} (johnsonEdition)`
        );
...
async function transformDefineProp(code, id, edition = "kevinEdition", isProduction = false) {
  ...
  await walkASTSetup(setupAst, (setup) => {
    setup.onEnter(
      (node, parent) => isCallOf(node, "$") && isCallOf(node.arguments[0], DEFINE_PROP3) || !isCallOf(parent, "$") && isCallOf(node, DEFINE_PROP3),
      (node, parent) => {
        const propName = walkCall(
          isCallOf(node, "$") && isCallOf(node.arguments[0], DEFINE_PROP3) ? node.arguments[0] : node,
          parent
        );
        s.overwriteNode(
          node,
    ...
    setup.onEnter(
      (node) => isCallOf(node, DEFINE_PROPS),
      (node) => {

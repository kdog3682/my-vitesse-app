                ...
                if (!deepLookup.found) {
                    if (deepLookup.path.length > 1) {
                        const deepPath = deepLookup.path
                            .map(i => node_path_1.default.relative(node_path_1.default.dirname((0, eslint_compat_utils_1.getPhysicalFilename)(context)), i.path))
                            .join(' -> ');
                        context.report({
                            node: imNode,
            ...
            VariableDeclarator(node) {
                ...
                for (const im of variableImports) {
                    ...
                    if (!deepLookup.found) {
                        if (deepLookup.path.length > 1) {
                            const deepPath = deepLookup.path
                                .map(i => node_path_1.default.relative(node_path_1.default.dirname((0, eslint_compat_utils_1.getFilename)(context)), i.path))
                                .join(' -> ');
                            context.report({
                                node: im.key,
                                messageId: 'notFoundDeep',

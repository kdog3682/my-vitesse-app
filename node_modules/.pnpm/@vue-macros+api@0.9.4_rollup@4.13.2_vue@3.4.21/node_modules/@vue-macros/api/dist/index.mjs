...
`);
  ...
  function buildNewProp(name, value, optional) {
    ...
    const valueAst = babelParse2(`type T = (${value})`, "ts").body[0].typeAnnotation.typeAnnotation;
    const signatureAst = babelParse2(`interface T {${signature}}`, "ts").body[0].body.body[0];
...
`);
  ...
  function parseSignature(signature) {
    return babelParse3(`interface T {${signature}}`, "ts").body[0].body.body[0];
  }
  async function resolveDefinitions(typeDeclRaw2) {
...
var RollupResolve = () => {
  ...
  const resolve = (ctx) => {
            ...
            for (const version of Object.values(pkg.typesVersions)) {
              for (const [entry, subpaths] of Object.entries(
                ...
                for (const subpath of subpaths) {
                  ...
                  if (isDts(resolved2) && existsSync2(resolved2))
                    return resolved2;
                }
              }
            }
    ...
    async function tryResolve(id, importer) {
      try {
